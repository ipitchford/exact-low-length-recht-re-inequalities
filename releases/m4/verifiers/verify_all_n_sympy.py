#!/usr/bin/env python3
"""Exact symbolic verifier for the Recht--Re m=4 certificate for all n>=4.
...
for which,sgn in [('upper',-1),('lower',1)]:
    for i,p in enumerate(PATTERNS):
        ...
        rhs=n*(n-1)*(n-2)*(n-3) if p==() else sp.Integer(sgn) if p==(0,1,2,3) else sp.Integer(0)
...
def interpolate_coeff_blocks(typ):
    samples=[direct_coeff_blocks(N,typ) for N in range(5,10)]
    out=[]
    for bi in range(4):
        ...
        for i in range(s):
            for j in range(s):
                for k in range(K):
                    C[i][j][k]=sp.interpolate([(N,sp.Integer(samples[N-5][bi][i][j][k])) for N in range(5,10)],n)
        out.append(C)
    # Independent extra point; degree <=4 follows from at most four freely summed indices.
    check=direct_coeff_blocks(10,typ)
    for bi,C in enumerate(out):
        for i in range(len(C)):
            for j in range(len(C)):
                for k in range(len(C[i][j])):
                    residual=sp.expand(C[i][j][k].subs(n,10)-check[bi][i][j][k])
                    require(
                        residual==0,
                        f'{typ} block {bi} coefficient ({i},{j}), orbit {k}: '
                        f'n=10 replay residual={residual}',
                    )
    return out

C_G=interpolate_coeff_blocks('G');C_H=interpolate_coeff_blocks('H')
print('BLOCK COEFFICIENTS: exact degree<=4 interpolation and independent n=10 check passed.')
...
def verify_seed_transversal(N,side,record,upper):
    ...
    b=sp.zeros(76,1)
    b[PI[()]]=sp.prod(N-j for j in range(4))
    b[PI[(0,1,2,3)]]=sp.Integer(-1 if side=='upper' else 1)
    if upper:
        C0=direct_coeff_blocks(N,'G')[0]
        K=sp.Matrix([[sum(C0[i][j][k] for j in range(8)) if k<59 else 0 for k in range(81)] for i in range(8)])
    ...
    require(
        ...
        f'{side} n={N} affine equations failed: residual={list(affine_residual)}',
    )
...
for which in ('upper','lower'):
    ...
    for typ,bi,M in blocks:
        ...
        for k in range(1,maxk+1):
            ...
            poly=sp.Poly(sp.expand(num.subs(n,t+5)),t,domain=sp.QQ)
...
print('POSITIVITY n>=5: 51 determinant polynomials and the upper kernel verified exactly.')

# ---------- separate n=4 certificates ----------
...
def direct_coeff_blocks4(typ):
    N=4;stab=typ=='G';om=G_ORB if stab else H_ORB;b=words(N);ids=[[om[canonical_entry(l,r,stab)] for r in b] for l in b]
    blocks=vectors_G4() if stab else vectors_H(4);K=59 if stab else 22;out=[]
    for vs in blocks:
        C=[[[0]*K for _ in vs] for _ in vs]
        for i,u in enumerate(vs):
            for j,v in enumerate(vs):
                for p,up in enumerate(u):
                    ...
                    for q,vq in enumerate(v):
                        if vq:C[i][j][ids[p][q]]+=up*vq
        out.append(C)
    return out

for which,sgn in [('upper',-1),('lower',1)]:
    x=[rat(z) for z in n4[which]['orbit_values']]
    for i,p in enumerate(PATTERNS):
        lhs=sum((sp.Integer(A0[i][j])+4*sp.Integer(A1[i][j]))*x[j] for j in range(81))
        rhs=24 if p==() else sgn if p==(0,1,2,3) else 0
        require(
            lhs==rhs,
            f'n=4 {which} endpoint identity failed for word pattern {p}: lhs={lhs}, rhs={rhs}',
        )
    blocks=[]
    for typ,off,K in [('G',0,59),('H',59,22)]:
        for bi,C in enumerate(direct_coeff_blocks4(typ)):
            s=len(C);M=sp.zeros(s)
            for i in range(s):
                for j in range(s):M[i,j]=sum(sp.Integer(C[i][j][k])*x[off+k] for k in range(K))
            blocks.append((typ,bi,M))
    for typ,bi,M in blocks:
        key=f'{which}_{typ}{bi}'
        if key=='upper_G0':
            ...
            require(
                ...
                f'n=4 upper G0 all-ones kernel failed: residual={list(kernel_residual)}',
            )
        ...
        for k in range(1,M.rows+1):
            ...
            require(
                determinant>0,
                f'n=4 {which} {typ}{bi} leading principal minor {k} is not positive: {determinant}',
            )
print('n=4: both exact identities and all representation-block positivity tests passed.')
print('VERIFIED: Recht--Re m=4 holds for every integer n>=4.')
