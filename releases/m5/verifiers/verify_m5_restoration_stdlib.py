#!/usr/bin/env python3
"""Dependency-free exact verifier for the Recht--Re m=5 restoration certificate.
...
Uses only Python's standard library and fractions.Fraction.  It verifies:
  ...
  * the 23 upper and 30 lower free rational functions and exact seed transversals;
  ...
  * the exact n=5 upper-endpoint certificate and its block positivity.
...
for w in ('upper','lower'):
    ...
    for row,patt in enumerate(PAT):
        ...
        rhs=mul([F(0)]*4+[F(1)],mul(mul(mul([F(0),F(1)],[-1,1]),mul([-2,1],[-3,1])),[-4,1])) if patt==() else ([F(0)]*4+[F(sgn)] if patt==(0,1,2,3,4) else [F(0)])
...
def count_blocks(N,typ,vfun=None):
    ...
    for vs in blocks:
        ...
        out.append(C)
    return out
SAMPLES={typ:[count_blocks(N,typ) for N in range(6,11)] for typ in ('G','H')}
# extra point verifies the combinatorial degree<=4 interpolation.
EXTRA={typ:count_blocks(11,typ) for typ in ('G','H')}
C={}
for typ in ('G','H'):
    C[typ]=[]
    for bi in range(4):
            ...
            for j in range(s):
                for k in range(K):
                    ...
                    require(
                        ...
                        f'n=11 replay mismatch, interpolated={replay_value}, direct={expected_value}',
                    )
        C[typ].append(block)
print('BLOCK COUNTS: degree<=4 interpolation and independent n=11 replay passed.')
...
def verify_seed_transversal(N,side,record,upper):
    ...
    b=[F(0)]*76
    b[PI[()]]=F(math.prod(N-j for j in range(5)))
    b[PI[(0,1,2,3,4)]]=F(-1 if side=='upper' else 1)
    if upper:
...
SEED_DATA=json.load(open(CERT/'base6_seed_certificates.json'))
verify_seed_transversal(6,'upper',SEED_DATA['upper'],True)
verify_seed_transversal(6,'lower',SEED_DATA['lower'],False)
ENDPOINT_DATA=json.load(open(CERT/'n5_upper_certificate.json'))
verify_seed_transversal(5,'upper',ENDPOINT_DATA['upper'],True)
...
print('POSITIVITY n>=6: block construction, 51 exact determinants, positive coefficients, and kernel passed.')
...
x=[F(z) for z in N5['upper']['orbit_values']]
for row,patt in enumerate(PAT):
    lhs=sum((F(A0[row][j]+5*A1[row][j])*x[j] for j in range(81)),F(0))
    rhs=F(120) if patt==() else F(-1) if patt==(0,1,2,3,4) else F(0)
    require(
        lhs==rhs,
        f'n=5 upper endpoint identity failed for word pattern {patt}: lhs={lhs}, rhs={rhs}',
    )
for typ,off,K,blocks in [('G',0,59,count_blocks(5,'G')),('H',59,22,count_blocks(5,'H'))]:
    for bi,CB in enumerate(blocks):
        sz=len(CB);M=[[sum((F(CB[i][j][k])*x[off+k] for k in range(K)),F(0)) for j in range(sz)] for i in range(sz)]
        if typ=='G' and bi==0:
            kernel_residuals=[sum(M[i],F(0)) for i in range(8)]
            require(
                all(z==0 for z in kernel_residuals),
                f'n=5 upper G0 all-ones kernel failed: residuals={kernel_residuals}',
            )
            Pm=[[F(1) if i==j else F(0) for j in range(7)] for i in range(7)]+[[-F(1)]*7]
            M=matmul(transpose(Pm),matmul(M,Pm));sz=7
...
# Exact specialization to the base n=6 certificates.
SEED=json.load(open(CERT/'base6_seed_certificates.json'))
for w in ('upper','lower'):
    vals=[]
    for num,c,q in FUN[w]:vals.append(sum(F(a)*F(6)**i for i,a in enumerate(num))/(F(c)*F(6)**q))
    expected_seed=[F(z) for z in SEED[w]['orbit_values']]
    require(
        vals==expected_seed,
        f'{w} parametric family does not specialize to the n=6 seed',
    )
print('SEED: parametric family specializes exactly to both n=6 certificates.')
print('VERIFIED (stdlib): Recht--Re m=5 upper bound holds for n>=5 and both bounds hold for n>=6.')
