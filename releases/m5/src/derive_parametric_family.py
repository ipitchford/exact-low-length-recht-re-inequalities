#!/usr/bin/env python3
"""Re-derive the all-n orbit functions from the exact n=6 seed certificate.

This is a discovery/provenance script, not needed by the two proof verifiers.
It implements balanced six-block averaging symbolically and confirms that the
...
def expected(u,v,base,stab):
 ...
 for colors in itertools.product(range(6),repeat=s):
  counts=[colors.count(a) for a in range(6)]
  weight=sp.prod(falling(n/6,c) for c in counts)
...
seed=json.load(open(CERT/'base6_seed_certificates.json'));published=json.load(open(CERT/'parametric_orbit_functions.json'))
C=falling(n,5)/falling(sp.Integer(6),5);r=n/6
for which in ('upper','lower'):
 ...
 print(f'{which}: all 81 orbit functions re-derived exactly from balanced six-block averaging.')
